...
const popupLangCardClass = (active: boolean) => (active ? "popup-lang-card active" : "popup-lang-card");
...
export const useAuthGlobalLoader = routeLoader$(({ cookie, url, redirect }) => {
  const token = cookie.get("token")?.value;
  const isAuth = !!token;
  
  const isAuthRoute = url.pathname.startsWith("/login") || url.pathname.startsWith("/register");
  
  if (!isAuth && !isAuthRoute && url.pathname !== "/") {
  ...
  return { 
    isAuth, 
    user: isAuth ? { email: "user@example.com", isTestPassed: false } : null 
  };
});
        ...
        <aside class="mobile-menu-drawer" aria-label={ui.mobileMenuLabel}>
          ...
          {sessionStore.isAuth && (
            <nav class="mobile-menu-list" aria-label="Main">
              ...
              {sessionStore.user?.isTestPassed === false && (
                <Link href="/profile" class="mobile-menu-item" style={{ color: "var(--color-primary, #6366f1)" }} onClick$={closeMobileMenu}>
                  <span class="mobile-menu-item-icon" aria-hidden="true">
                    <svg viewBox="0 0 24 24" role="presentation"><path d="M12 3.75C10.5 6 9 9 9 12s1.5 6 3 8.25" /><path d="M12 3.75C13.5 6 15 9 15 12s-1.5 6-3 8.25" /><path d="M3.75 12h16.5" /></svg>
                  </span>
                  <span class="mobile-menu-item-label">{ui.homeActionPlacementTitle}</span>
                </Link>
              )}
        ...
        <div class="topbar-inner">
                      ...
                      <div class="lang-popup-chips">
                        {LEARNING_LANGUAGE_OPTIONS.map((option) => (
                          <Link
                            ...
                            href={buildLanguageHref(location.url, option.code)}
                            class={popupLangCardClass(option.code === currentLanguage)}
                            aria-current={option.code === currentLanguage ? "page" : undefined}
                          >
                            <span class="popup-lang-code">{option.code.toUpperCase()}</span>
                            <span class="popup-lang-name">{option.nativeName}</span>
                          </Link>
                        ))}
                      </div>
                    </div>
                      ...
                      <div class="lang-popup-chips">
                        {UI_LANGUAGE_OPTIONS.map((option) => (
                          <Link
                            ...
                            href={buildUiLanguageHref(location.url, option.code)}
                            class={popupLangCardClass(option.code === currentUiLanguage)}
                            aria-current={option.code === currentUiLanguage ? "page" : undefined}
                          >
                            <span class="popup-lang-code">{option.code.toUpperCase()}</span>
                            <span class="popup-lang-name">{option.nativeName}</span>
          ...
          {sessionStore.isAuth && (
            <nav class="nav nav-desktop" aria-label="Main">
              ...
              {sessionStore.user?.isTestPassed === false && (
                <Link href="/profile" class="nav-link nav-link-highlight" style={{ color: "var(--color-primary, #6366f1)", fontWeight: "600" }}>
                  <span class="nav-icon" aria-hidden="true">
                    <svg viewBox="0 0 24 24" role="presentation"><circle cx="12" cy="12" r="8.25" /><path d="M12 3.75C10.5 6 9 9 9 12s1.5 6 3 8.25" /></svg>
                  </span>
                  <span class="nav-label">{ui.homeActionPlacementTitle}</span>
                </Link>
              )}
